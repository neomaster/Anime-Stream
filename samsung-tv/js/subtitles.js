var Subtitles = (function () {
  ...
  function getAudioPref() {
    try { return localStorage.getItem(AUDIO_KEY) || 'legendado'; }
    catch (e) { return 'legendado'; }
  }

  function saveAudioPref(value) {
    try { localStorage.setItem(AUDIO_KEY, value); } catch (e) { /* ignore */ }
  }
  ...
  function saveTrackPref(index) {
    try { localStorage.setItem(SUB_KEY, String(index)); } catch (e) { /* ignore */ }
  }

  function getTrackPref() {
    try {
      var v = localStorage.getItem(SUB_KEY);
